...
@section('caseContent')
    <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
        <div class="bg-light mt-3">
            <label class="h3 font-weight-bold mt-3 mx-4">मस्यौदा</label>
            (<b>Case Number</b>:{{ $cases->case_number }})
            (<b>Case Type</b>:{{ $cases->case_type }})
            (<b>Case Status</b>:{{ $cases->case_status }})
            <a href="{{ route('draft.create', $cases) }}" class="btn btn-info float-right mx-5">Add</a>
        </div>
                    ...
                    @foreach ($consultations as $item)
                        <tr>
                            ...
                            <td>
                                ...
                                <a class="action-btn text-primary" href="{{ route('draft.edit', $item) }}"><i
                                        class="far fa-edit"></i></a>
                                <form action="{{ route('draft.destroy', $item) }}" method="post"
